...
class InvoiceGenerator:
   def __init__(self,root):
       ...
       # ====submit details
       Button(self.frame, text="Save Invoice", command=self.save_invoice, font=("times new roman", 14),
              fg="white",
              cursor="hand2", bg="#B00857").place(x=50, y=600, width=180, height=40)
   ...
   def generate_invoice(self, file_path):
       c = canvas.Canvas(file_path, pagesize=(200, 250), bottomup=0)
       ...
       c.drawCentredString(125, 35, self.city.get() + ", Netherlands")
...
## Need to ask if we need the invoice num ber will be manually inputted or if he wants the system to automate it
## Also need to see if we can be flexible with the country of origin
